    ...
    const fetchData = async () => {
        try {
            ...
            function renderTopOffers() {
                const topOffers = products.filter(p => p.discount > 0);
                document.getElementById("product-container").innerHTML = cardDisplay(topOffers);
            };
            ...
            control.querySelector('.increaseBtn').addEventListener('click', () => {
                input.value = parseInt(input.value || 0) + 1;
            });
            control.querySelector('.decreaseBtn').addEventListener('click', () => {
                input.value = Math.max(1, parseInt(input.value || 0) - 1);
            });
            // Event delegation for buttons
            const topOfferContainer = document.getElementById("top-offer-product-display");
            topOfferContainer?.addEventListener("click", function (e) {
                const cartBtn = e.target.closest(".addToCartBtn");
                const listBtn = e.target.closest(".addToBuyingList");

                if (cartBtn) toggleButton(cartBtn);
                if (listBtn) toggleButton(listBtn);
            });
