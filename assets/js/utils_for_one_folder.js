const headerSection = `
    ...
    function toggleMobileSubmenu(id) {
        ...
        // You can add icon rotation here if needed
    }
    ...
    // Allow multiple Add to Cart buttons to work independently
    document.querySelectorAll('.addToCartBtn').forEach((btn) => {
    btn.addEventListener('click', function () {
        const isAdded = btn.classList.contains('bg-[#318D23]');

        // Toggle styles only, keep icon the same
    ...
    currencyDropdown.querySelectorAll('button[data-currency]').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // Optionally: Save currency preference
        });
    });
