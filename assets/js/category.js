...
const fetchCategoryProducts = async () => {
    try {
        ...
        const categoryId = params.get('categoryId') || params.get('id');
        
        if (!categoryId) {
            document.getElementById("products-container").innerHTML = "<p>No category ID found.</p>";
            return;
        }
        ...
        document.getElementById("products-container").innerHTML = `
            <div class="flex justify-center items-center py-8">
                <div class="text-lg">Loading products...</div>
            </div>
        `;
...
const fetchCategoryDetails = async (categoryId) => {
    try {
        const res = await fetch(`${BASE_URL}/api/v1/categories/${categoryId}?page=0&size=100`);
        if (!res.ok) throw new Error("Failed to fetch category details");
        const category = await res.json();

        // Update page title and category info
        document.title = `${category.name} - Products`;
        
        // Update category header if it exists
        const categoryHeader = document.getElementById("category-header");
        if (categoryHeader) {
            categoryHeader.innerHTML = `
                <h1 class="text-2xl md:text-3xl font-bold text-[#21242A] mb-2">${category.name}</h1>
                ${category.description ? `<p class="text-gray-600 dark:text-gray-400">${category.description}</p>` : ''}
            `;
        }
...
const fetchProducts = async (categoryId) => {
    try {
        const res = await fetch(`${BASE_URL}/api/v1/products/get-by-category/${categoryId}?page=0&size=100`);
        ...
        if (products.length > 0) {
            ...
            document.getElementById("products-container").innerHTML = `
                <div class="col-span-full text-center py-8">
                    <p class="text-lg text-gray-800 text-center">No products found in this category.</p>
                    <img src="https://i.postimg.cc/d0PgpGqy/Empty-cuate.png" alt="empty" class="w-full h-auto max-w-sm flex justify-center">
                </div>
            `;
        }
        ...
        document.getElementById("products-container").innerHTML = `
            <div class="col-span-full text-center py-8">
                <p class="text-lg text-red-500">Error loading products. Please try again.</p>
            </div>
        `;
    }
};
...
const setupSearch = () => {
    ...
    if (searchInput) {
        ...
        // Modify the original fetchProducts to store products
        const originalFetchProducts = fetchProducts;
        fetchProducts = async (categoryId) => {
            await originalFetchProducts(categoryId);
            
            // Get the products from the DOM or make another API call
            const res = await fetch(`${BASE_URL}/api/v1/products?categoryId=${categoryId}`);
            const response = await res.json();
            let products = response.content || response.data || response.products || response;
            ...
            switch (sortType) {
                ...
                case 'price-asc':
                    sortedProducts.sort((a, b) => {
                        const priceA = Number(a.priceOut) - (Number(a.discount) || 0);
                        const priceB = Number(b.priceOut) - (Number(b.discount) || 0);
