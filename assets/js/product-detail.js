...
const fetchData = async () => {
    try {
        ...
        if (!productId) {
            document.getElementById("product-detail-section").innerHTML = "<p>No product ID found.</p>";
            return;
        }
        ...
        // Display product detail section
        document.getElementById("product-detail-section").innerHTML = productDetailSection(product);

        // Fetch and display related products from the same category
        if (product.category?.uuid) {
            await fetchRelatedProducts(product.category.uuid, productId);
        }
        ...
        document.getElementById("product-detail-section").innerHTML = "<p>Error loading product details.</p>";
    }
};

const fetchRelatedProducts = async (categoryId, currentProductId) => {
    try {
        // Try different possible endpoints for getting products by category
        const possibleEndpoints = [
            `${BASE_URL}/api/v1/products/get-by-category/${categoryId}?page=0&size=100`,
        ];
        ...
        for (const endpoint of possibleEndpoints) {
            try {
                ...
                if (response.content && Array.isArray(response.content)) {
                    relatedProducts = response.content;
                    successful = true;
                    break;
        ...
        if (!successful) {
            console.log('None of the endpoints returned products. Available endpoints tried:');
            possibleEndpoints.forEach(endpoint => console.log(`- ${endpoint}`));
            return;
        }
        
        console.log('Processed related products:', relatedProducts);
        
        // Filter out the current product and limit to 6 related products
        const filteredProducts = relatedProducts
            .filter(product => product.uuid !== currentProductId)
            .slice(0, 6);
        ...
        if (filteredProducts.length > 0) {
            ...
            console.log('No related products found after filtering');
        }
...
const displayRelatedProducts = (products) => {
    ...
    // Generate product cards using .map()
                ...
                <div class="mt-2 md:mt-4 w-full flex flex-col gap-2 items-center">
                    ...
                    <div class="w-full flex items-center gap-1">
                        <button type="button" 
                            ...
                            data-product-id="${pro.uuid}" data-product-name="${pro.name}" data-product-price="${pro.priceOut}">
    ...
    // Populate the related products container
    const relatedProductsContainer = document.getElementById("retalted-products") || 
                                   document.getElementById("related-products") ||
                                   document.querySelector(".related-products-container");
    
    if (relatedProductsContainer) {
        relatedProductsContainer.innerHTML = productCardsHTML;
        console.log('Related products populated successfully');
    } else {
        console.error('Related products container not found. Looking for: #retalted-products, #related-products, or .related-products-container');
        // List all possible containers for debugging
        console.log('Available elements with "related" in ID or class:');
        document.querySelectorAll('[id*="related"], [class*="related"]').forEach(el => {
            console.log(`- ${el.tagName} with id="${el.id}" and class="${el.className}"`);
        });
    }
};

function productDetailSection(pro) {
    return `
        <div class="w-full md:w-2/3 border-2 border-[#e5e5e5] dark:border-[#222831] rounded-lg bg-white dark:bg-[#393E46] grid grid-cols-1 gap-4 p-4 md:p-8">
            <!-- Main Image -->
            <div class="w-full">
                <img src="${pro.thumbnail || pro.color?.[0]?.images?.[0] || 'https://upload.wikimedia.org/wikipedia/commons/1/14/No_Image_Available.jpg'}" alt="${pro.name}"
                class="w-full h-66 md:h-[360px] 2xl:h-[480px] object-cover rounded-lg border-2 border-[#e5e5e5] dark:border-[#222831]"
                />
            </div>
            <div class="w-full grid grid-cols-2 gap-4">
                <img src="${pro.thumbnail || 'https://upload.wikimedia.org/wikipedia/commons/1/14/No_Image_Available.jpg'}" alt="${pro.name}"
                class="w-full object-cover h-36 md:h-[160px] 2xl:h-[228px] rounded-lg border-2 border-[#e5e5e5] dark:border-[#222831]"/>
            </div>
        </div>
        ...
        <div class="w-full flex flex-col gap-6 ">
            <div class="w-full flex flex-col gap-2">
                <h2 class="text-[1rem] md:text-[1.5rem] text-[#21242A] font-semibold">${pro.name || 'No Name Available'}</h2>
                <p class="text-gray-500 text-[1rem] md:text-[20px]">Brand: ${pro.brand?.name || 'Unknown Brand'}</p>
                <p class="text-gray-500 text-[1rem] md:text-[20px]">Supplier: ${pro.supplier?.name || 'Unknown Supplier'}</p>
            ...
            <p class="text-gray-500 text-[1rem] md:text-[20px]">Stock: ${pro.stockQuantity || 'Sold out'}</p>
            <div class="w-full container flex items-center max-w-xs quantity-control">
                <button type="button" 
                    class="decreaseBtn text-[1rem] md:text-[1.4rem] text-[#21222A] border border-gray-200 px-2 rounded-l-md hover:bg-[#f5f5f5] transition-colors duration-300 h-8 md:h-10">
                    <i class="fa-solid fa-minus"></i>
                    <span class="sr-only">Decrease quantity</span>
                </button>

                <input type="text" 
                    class="quantityInput text-center border-t border-b bg-white dark:bg-[#31363F] border-gray-300 w-12 h-8 md:h-10" 
                    value="1" inputmode="numeric" pattern="[0-9]*" placeholder="Qty" />

                <button type="button" 
                    class="increaseBtn text-[1rem] md:text-[1.4rem] text-[#21222A] border border-gray-200 px-2 rounded-r-md hover:bg-[#f5f5f5] transition-colors duration-300 h-8 md:h-10">
                    <i class="fa-solid fa-plus"></i>
                    <span class="sr-only">Increase quantity</span>
                </button>
            </div>
            ...
            <div class="w-full flex flex-col md:flex-row justify-between gap-4">
                <button type="button" class="addToCartBtn w-full text-[1rem] md:text-[1.5rem] text-[#318D23] border border-[#318D23] px-6 py-3 rounded-full hover:bg-[#318D23] hover:text-white transition-colors duration-300" data-product-id="${pro.uuid}">
                    Add to Cart <i class="fa-solid fa-cart-plus"></i>
                </button>
                <button type="button" class="addToBuyingList w-full text-[1rem] md:text-[1.5rem] text-[#318D23] border border-[#318D23] px-6 py-3 rounded-full hover:bg-[#318D23] hover:text-white transition-colors duration-300">
                    Add to Buying List <i class="fa-solid fa-clipboard-list"></i>
                </button>
            </div>

            <!-- Toggle Buttons -->
            <div class="flex justify-start gap-4 mt-6 md:mt-10 border-b border-gray-200 dark:border-gray-600">
                <button id="descTab" class="tab-btn text-[#21242A] border-b-2 border-[#21242A] font-semibold pb-2" type="button">Description</button>
                <button id="reviewTab" class="tab-btn text-gray-500 dark:text-gray-400 hover:text-[#318D23] pb-2" type="button">Review</button>
            </div>
            
            <!-- Tab Content -->
            <div class="mt-6 md:mt-8">
                <!-- Description Content -->
                <div id="descriptionSection" class="tab-content">
                    <h2 class="text-lg md:text-[1.5rem] font-semibold mb-2">Product Description</h2>
                    <p class="text-[1rem] md:text-[1.2rem] text-gray-700 dark:text-gray-300 leading-relaxed">
                    ${pro.description || 'No description available'}
                    </p>
                </div>

                <!-- Review Content -->
                <div id="reviewSection" class="tab-content hidden">
                    <h3 class="text-lg md:text-[1.5rem] font-semibold mb-4">
                    Customer Reviews
                    <i class="fa-solid fa-star text-[#F8DB23]"></i>
                    <i class="fa-solid fa-star text-[#F8DB23]"></i>
                    <i class="fa-solid fa-star text-[#F8DB23]"></i>
                    <i class="fa-solid fa-star text-[#F8DB23]"></i>
                    <i class="fa-solid fa-star text-[#F8DB23]"></i>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="border-b border-gray-200 dark:border-gray-600 pb-4">
                            ...
                            <i class="fa-solid fa-star text-[#F8DB23]"></i>
                            <i class="fa-solid fa-star text-[#F8DB23]"></i>
                            Soriya23536 <span class="text-sm text-gray-500">– Jun 21, 2025</span>
                            </p>
                            <p>Very good service</p>
                            <p class="text-gray-500">Delivery took only 8 hours to my location.</p>
                        </div>
                    </div>
                </div>
            </div>
