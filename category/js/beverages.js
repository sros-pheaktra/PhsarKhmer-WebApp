                    ...
                    <div id="mobile-category-menu" class="hidden pl-4 mt-2 border-l-2 border-[#318D23]">
                        ...
                        <div class="mb-2">
                            ...
                            <div id="mobile-beverages" class="hidden pl-4 mt-1">
                                ...
                                <a href="../category.html?categoryId=928a9508-58d6-4bfa-8154-6a1420f49151" class="block py-1 px-2 rounded hover:bg-[#D8FDC9] dark:hover:bg-[#1B4242] dark:bg-[#1B4242] text-[#21242A] dark:text-gray-100">Water</a>
                            </div>
                        </div>
...
const fetchData = async () => {
    try {
        ...
        let beverages = products.filter(
            p => {
                const name = p.category?.name?.toLowerCase();
                return name === "beverages" || name === "milk" || name === "energy drink" || name === "fruit juice" 
                    || name === "powdered drink mixed" || name === "soft drink" || name === "tea" || name === "water"
            }
        );

        // ✅ Render only beverages
        const cardDisplay = beverages.map(pro => `
        ...
        document.getElementById("products-container").innerHTML = cardDisplay.join("");
...
function initializeEventListeners() {
    ...
    const existingListeners = document.querySelectorAll('[data-listeners-attached]');
    existingListeners.forEach(el => el.removeAttribute('data-listeners-attached'));

    // ✅ Quantity controls - Direct event listeners
    document.querySelectorAll('.quantity-control').forEach(control => {
        if (control.hasAttribute('data-listeners-attached')) return;
        ...
        control.setAttribute('data-listeners-attached', 'true');
    });

    // ✅ Add to cart buttons - Direct event listeners
    document.querySelectorAll('.addToCartBtn').forEach(btn => {
        if (btn.hasAttribute('data-listeners-attached')) return;
        ...
        btn.addEventListener('click', (e) => {
            ...
            const quantityInput = btn.closest('.container').querySelector('.quantityInput');
    ...
    // ✅ Add to buying list buttons - Direct event listeners  
    document.querySelectorAll('.addToBuyingList').forEach(btn => {
        if (btn.hasAttribute('data-listeners-attached')) return;
        ...
        btn.addEventListener('click', (e) => {
            ...
            // Add your buying list logic here
            console.log('Added to buying list:', {
                id: productId,
                name: productName,
                price: productPrice,
                quantity: quantity
            ...
            // You can add actual buying list functionality here
            // addToBuyingList(productId, quantity);
        });
        
        btn.setAttribute('data-listeners-attached', 'true');
    });

    // ✅ Tab switching (if elements exist)
    ...
    if (descTab && reviewTab && descSection && reviewSection) {
        descTab.addEventListener('click', (e) => {
            e.preventDefault();
            // Show description, hide review
...
function toggleButtonState(btn) {
    ...
    if (isAdded) {
        // Remove active state
        btn.classList.remove('bg-[#318D23]', 'text-white');
        btn.classList.add('text-[#318D23]');
    } else {
        // Add active state
        btn.classList.add('bg-[#318D23]', 'text-white');
        btn.classList.remove('text-[#318D23]');
    }
    
    console.log('Button state toggled:', isAdded ? 'removed' : 'added');
}
